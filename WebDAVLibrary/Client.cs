...
namespace WebDAVLibrary
{
    public class Client : IClient
    {
        ...
        private async Task<Item> Get(Uri listUri, string path)
        {
            ...
            try
            {
                ...
                if (response.StatusCode != HttpStatusCode.OK &&
                    (int)response.StatusCode != HttpStatusCode_MultiStatus)
                {
                    WebDAVException ex = new WebDAVException((int)response.StatusCode, string.Format("Failed retrieving item/folder (Status Code: {0})", response.StatusCode));
                    throw ex;
                }
                ...
                using (var stream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
                {
                    ...
                    if (result == null)
                    {
                        WebDAVException ex = new WebDAVException("Failed deserializing data returned from server.");
                        throw ex;
                    }
        ...
        public async Task<Stream> FilePreviewDownload(string remoteFilePath)
        {
            // Should not have a trailing slash.
            var downloadUri = await GetServerUrl(remoteFilePath, false).ConfigureAwait(false);

            Uri myUri = new Uri(downloadUri.Uri, "?preview&size=XS");
            //var url = new Uri(remoteFilePath);
            //  var headers = new Dictionary<string, string> { { "translate", "f" } };

            // var response = await HttpRequest(url, HttpMethod.Get, dictionary).ConfigureAwait(false);
            using (var request = new HttpRequestMessage(HttpMethod.Get, myUri.PathAndQuery))
            {
                request.Headers.Connection.Add("Keep-Alive");
                request.Headers.UserAgent.Add(new ProductInfoHeaderValue("WebDAVClientAronovAleks13PRI", AssemblyVersion));
                request.Headers.Host = "webdav.yandex.ru";
                ////// RFC6749
                //request.Headers.Authorization = "OAuth 0c4182a7c2cf4521964a72ff57a34a07";
